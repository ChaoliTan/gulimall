...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void up(Long spuId) {
        ...
        List<SkuEsModel.AttrsEsModel> attrsEsModelList = baseAttrs.stream().filter(item -> {
            return idSet.contains(item.getAttrId());
        }).map(item -> {
            SkuEsModel.AttrsEsModel attrs = new SkuEsModel.AttrsEsModel();
        ...
        List<SkuEsModel> uoProducts = skus.stream().map(sku -> {
            ...
            // TODO 4. 设置检索属性
            esModel.setAttrs(attrsEsModelList);
